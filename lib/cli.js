...
const cli = cac('create-client-app')
...
const showEnvInfo = async () => {
  ...
  const result = await envinfo
    .run({
      ...
      npmGlobalPackages: ['nuxt', 'create-client-app']
    })
  console.log(result)
  process.exit(1)
}
...
cli
  .command('[out-dir]', 'Generate in a custom directory or current (empty) directory')
  ...
  .action((outDir = '.', cliOptions) => {
    ...
    console.log()
    console.log(chalk`{cyan create-client-app v${version}}`)
    if (files.length) {
